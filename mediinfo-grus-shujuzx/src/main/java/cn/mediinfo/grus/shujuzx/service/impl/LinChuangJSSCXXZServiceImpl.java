...
//        for (var item :shuChuDto){
//            for (var scmx :item.getZiDuanList()){
...
//        }
        shuChuDto.forEach(x -> {
            if (!CollectionUtils.isEmpty(x.getZiDuanList())) {
                shuChuBXDto.addAll(x.getZiDuanList().stream().filter(m -> Objects.equals(m.getShuChuBXBZ(), 1)).toList());
            }
        });
    ...
    public List<ShiTuMXZHCXDto> getShiTuMXZHCX(List<SC_CX_ShiTuXXModel> shiTuXXList, List<SC_CX_ShiTuMXModel> shiTuMXList, List<LingChuangJSPZZDXXRso> lingChuangJSPZZDList, List<GuanLianTJZD> shiTuMXGXList) {
        ...
        for (SC_CX_ShiTuXXModel e : shiTuXXList) {
            ...
            if (dto != null && !shiTuMXByShiTuIdList.isEmpty()) {
                ...
                //远程返回的数据
                //远程返回的数据拷贝一个list 并过滤明细
                List<ShuJuJMXZDDto> list = dto.getShiTuMXZDDtos().stream().filter(p->mingXiZDBMList.contains(p.getZiDuanBM())).toList();
                list.forEach(b -> {
                    SC_CX_ShiTuMXModel ziDuanXX = shiTuMXByShiTuIdList.stream().filter(z -> z.getZiDuanBM().equals(b.getZiDuanBM())).findFirst().orElse(null);
                    if (ziDuanXX != null) {
                        b.setTiaoJianBZ(ziDuanXX.getTiaoJianBZ());
                        b.setShuChuBZ(ziDuanXX.getShuChuBZ());
                        b.setShuChuBXBZ(ziDuanXX.getShuChuBXBZ());
                    }
                });
                var copyList = BeanUtil.copyListProperties(list, ShuJuJMXZDDto::new);
